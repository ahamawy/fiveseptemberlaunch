...
export function V(name?: string): Record<string, any> {
  ...
  const viewport = { once: (motionCfg as any).scroll?.viewportOnce ?? true, amount: (motionCfg as any).scroll?.amount ?? 0.2 };
...
export function formatNumber(value: number, format?: string): string {
  ...
  if (format.includes('0,0a')) {
    ...
    if (abs >= 1_000_000_000) { v = v / 1_000_000_000; unit = 'b'; }
    else if (abs >= 1_000_000) { v = v / 1_000_000; unit = 'm'; }
    else if (abs >= 1_000) { v = v / 1_000; unit = 'k'; }
  }
  const num = new Intl.NumberFormat('en-US', { maximumFractionDigits: 1 }).format(v);
