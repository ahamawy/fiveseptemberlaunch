...
export class EnhancedFeeCalculator {
  ...
  async loadActiveFeeSchedule(dealId: number): Promise<FeeScheduleRow[]> {
    ...
    if (error) {
      throw new Error(`Failed to load fee schedule: ${error.message}`);
    }
    ...
    return (data || []).map(row => ({
      ...
      basis: row.basis as 'GROSS' | 'NET' | 'NET_AFTER_PREMIUM',
      precedence: row.precedence,
      effective_at: new Date(row.effective_at),
      version: row.schedule_version?.version
    }));
  }
  ...
  applyDiscounts(
    ...
    for (const discount of discounts) {
      ...
      state.appliedFees.push({
        ...
        applied: true,
        notes: `Discount on ${baseComponent}: ${discount.percent ? (discount.percent * 100) + '%' : '$' + discount.amount}`
      });
  ...
  applyAnnualFees(
    ...
    for (const annual of annualFees) {
      const fee = state.appliedFees.find(f => f.component === annual.component);
      if (!fee) continue;
  ...
  validateCalculation(
    ...
    if (Math.abs(state.netAmount - expectedNet) > 0.01) {
      errors.push(`Net amount mismatch: expected ${expectedNet}, got ${state.netAmount}`);
    }
    
    // 3. Verify discounts are negative
    const positiveDiscounts = state.appliedFees
      .filter(f => f.component.includes('_DISCOUNT') && f.amount >= 0);
    if (positiveDiscounts.length > 0) {
      errors.push(`Discounts must be negative: ${positiveDiscounts.map(d => d.component).join(', ')}`);
    }
    
    // 4. Verify transfer reconciliation
    const calculatedPost = finalAmounts.transferPreDiscount - finalAmounts.totalDiscounts;
    if (Math.abs(calculatedPost - finalAmounts.transferPostDiscount) > 0.01) {
      errors.push(`Transfer reconciliation failed: ${calculatedPost} != ${finalAmounts.transferPostDiscount}`);
    }
    ...
    if (unroundedAmounts.length > 0) {
      warnings.push(`Unrounded amounts detected: ${unroundedAmounts.length} fees`);
    }
    ...
    if (firstPositiveFee && firstPositiveFee.component !== 'PREMIUM') {
      warnings.push(`Premium should be first fee, found: ${firstPositiveFee.component}`);
    }
  ...
  async persistCalculation(
    ...
    if (!calculation.validation.valid) {
      throw new Error(`Cannot persist invalid calculation: ${calculation.validation.errors.join(', ')}`);
    }
    
    const supabase = this.client.getClient();
    
    // Prepare records with transaction ID
    const records = calculation.state.appliedFees.map(fee => ({
      ...fee,
   ...
   */
  async calculateForTransaction(transactionId: number): Promise<FeeCalculationResult> {
    const supabase = this.client.getClient();
  ...
  async batchCalculate(
    ...
    for (const [dealId, dealTxs] of byDeal) {
      ...
      for (const tx of dealTxs) {
        ...
        let state = this.calculateFeesWithPrecedence(schedule, tx.gross_capital);
