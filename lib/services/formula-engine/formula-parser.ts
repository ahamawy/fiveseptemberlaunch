...
export class FormulaParser {
  ...
  private replaceVariables(formula: string): string {
    ...
    const sortedVars = Array.from(this.context.variables.entries()).sort(
...
export class FormulaExecutor {
  ...
  async calculate(
    ...
    const varArray: FormulaVariable[] = Object.entries(variables).map(
      ([code, value]) => ({
        code,
        value: value || 0,
      })
  ...
  async calculateDealEconomics(
    ...
    const updatedVars: Record<string, number> = {
      ...variables,
      NC: netCapital,
    };
    ...
    if (formulas.eq_proceeds_formula) {
      eqProceeds = await this.calculate(
        formulas.eq_proceeds_formula,
        updatedVars
      );
