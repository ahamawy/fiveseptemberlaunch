...
export function buildLegacyProfileFromMapping(mapping: ExtractedDealData): any {
  ...
  const premiumPct =
    mapping?.deal?.premium_percent ??
    (pre && sell ? ((sell - pre) / pre) * 100 : undefined);
  return {
    ...
    components: {
      PREMIUM:
        premiumPct != null
          ? {
              method: "PCT_OF_GROSS",
              percent: Number(premiumPct.toFixed(5)),
              direction: "DEDUCTS_NET",
            }
          : undefined,
      STRUCTURING: {
        method: "PCT_OF_GROSS",
        percent: 4,
        direction: "ADDS_TRANSFER",
      },
      MANAGEMENT: {
        method: "PCT_OF_GROSS",
        percent: 2,
    ...
    valuation:
      pre && sell
        ? {
            purchase_pre_money: pre,
            sell_pre_money: sell,
            premium_percent_formula: "(sell - purchase)/purchase",
          }
        : undefined,
  };
}
