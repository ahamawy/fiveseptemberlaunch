...
export class MockAdapter implements IDataClient {
  ...
  async createDeal(input: {
    ...
    const nextId = (this.deals.reduce((m, d) => Math.max(m, d.id), 0) || 0) + 1;
    const now = new Date().toISOString();
    const deal: Deal = {
      id: nextId,
      public_id: `deal_${nextId}`,
      code: `DEAL-${nextId}`,
      slug: input.slug,
      name: input.name,
  ...
  async getPortfolioData(investorId: number): Promise<PortfolioData> {
    ...
    const holdings = await Promise.all(
      signedCommitments.map(async (commitment) => {
        ...
        const company = deal?.company_id
          ? await this.getCompanyById(deal.company_id)
          : null;
        const transactions = await this.getTransactions({
          investor_id: investorId,
    ...
    const totalValue = holdings.reduce(
      (sum, h) => sum + h.metrics.currentValue,
      0
    );
    const totalCalled = holdings.reduce(
      (sum, h) => sum + h.metrics.capitalCalled,
      0
    );
    const totalDistributed = holdings.reduce(
      (sum, h) => sum + h.metrics.capitalDistributed,
      0
    );
    const totalGains = totalValue + totalDistributed - totalCalled;
    const averageIRR =
      holdings.reduce((sum, h) => sum + (h.metrics.irr || 0), 0) /
      holdings.length;
