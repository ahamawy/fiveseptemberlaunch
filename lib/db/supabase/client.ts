...
export class SupabaseDirectClient {
  ...
  constructor(config: SchemaConfig) {
    ...
    const key = config.getSupabaseServiceKey() || config.getSupabaseAnonKey();
    this.client = createClient(config.getSupabaseUrl(), key, {
      auth: {
  ...
  async executeSQL<T = any>(query: string, params?: any): Promise<T> {
    ...
    console.warn(
      "executeSQL is deprecated. Use query builder or views instead."
    );
    throw new Error(
      "Direct SQL execution not available. Use query builder or views."
    );
  }
